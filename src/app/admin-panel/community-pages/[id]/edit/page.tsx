...
export default function EditCommunityPagePage() {
  ...
  /* ── Set editor content once both data and editor are ready ──────────────── */
  useEffect(() => {
    if (editor && editorReady && pageData && pageData.content) {
      editor.commands.setContent(pageData.content);
    }
  }, [editor, editorReady, pageData]);
  ...
  async function handleEditorImageFileSelect(
    ...
    try {
      ...
      setToastState({ message: "Editor image upload failed", type: "error" });
    } finally {
            ...
            <div>
              ...
              {slug !== pageData.slug && (
                <p className="text-amber-400 text-xs mt-1">
                  ⚠ Changing the slug will change the URL from /{pageData.slug}{" "}
                  to /{slug}
                </p>
              )}
            </div>
